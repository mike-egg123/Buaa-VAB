...
document.getElementById("tree").onresize = function() {
  let old_width = W;
  setSize();
  populate();
};
...
class Leaf {
  constructor(parent, length) {
    ...
    this.end.angle = 0;
    this.z_value = 0;
    this.colors = ["#ff6347", "#FDE5E5", "#e34e7c"];
...
class Plant {
  constructor(x, y, l) {
    ...
    this.min_thickness = 4;
    this.max_thickness = 13;
    this.thickness =
      Util.map(this.length, H, 0, this.min_thickness, this.max_thickness) *
      Util.random(0.8, 1.2);
    this.z_value = 0;
    this.colors = ["#2c2a4d", "#562a3b"];
    this.true_color = this.colors[
      Math.floor(Math.random() * this.colors.length)
    ];
  }
...
let wind = {
  ...
  reset: function() {
    ...
    this.duration = Util.random(2000, 5000);
